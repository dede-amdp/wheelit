...
class _LoginState extends State<LoginScreen> {
  ...
  login()async {
    if(_formKey.currentState.validate()) {
      ...
      try{
        User user = (await _auth.signInWithEmailAndPassword(email: _email, password: _password)) as FirebaseUser;
      }
      catch(e){
        showError(e.message);
        print(e);
                    ...
                    child: Column(
                      children: <Widget>[
                        Container(
                          child: TextFormField(
                              validator: (input) {
                                emailValid = RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(input);
                                if(input.isEmpty) {
                                  return '  Enter Email';
